...
const INDEX_CONFIG = {
  NIFTY:      { instrument_key: 'NSE_INDEX|Nifty 50',           label: 'Nifty 50',     lot: 65,  gap: 50  },
  BANKNIFTY:  { instrument_key: 'NSE_INDEX|Nifty Bank',          label: 'Bank Nifty',   lot: 30,  gap: 100 },
  FINNIFTY:   { instrument_key: 'NSE_INDEX|Nifty Fin Service',   label: 'Fin Nifty',    lot: 60,  gap: 50  },
...
function parseNSE(nseData, lot) {
  ...
  (fil.data||rec.data||[]).forEach(r=>{
    const K=parseFloat(r.strikePrice), exp=r.expiryDate;
    if(!byStrike[K]) byStrike[K]={};
    if(!byStrike[K][exp]) byStrike[K][exp]={ce:r.CE||{},pe:r.PE||{}};
  });
  const rows = Object.keys(byStrike).sort((a,b)=>a-b).map(K=>{
    const first=Object.values(byStrike[K])[0], ce=first.ce, pe=first.pe;
    return {
      strike:parseInt(K), c_key:ce.identifier||`NSE_FO|NIFTY${K}CE`, p_key:pe.identifier||`NSE_FO|NIFTY${K}PE`,
...
export default function OptionChain() {
  ...
  const initIdx  = router.query.index || 'NIFTY';

  const [indexKey, setIndexKey] = useState(initIdx);
  ...
  useEffect(() => {
    setIndexKey(router.query.index || 'NIFTY');
  }, [router.query.index]);
  ...
  async function fetchExpiries() {
    ...
    try {
      const r = await fetch(`/api/expiry?instrument_key=${encodeURIComponent(cfg.instrument_key)}`,
        { headers: token ? { Authorization:`Bearer ${token}` } : {} });
      const d = await r.json();
      if (d.expiries?.length) { setExpiries(d.expiries); if (!expiry) setExpiry(d.expiries[0]); }
    } catch {}
  }
  ...
  async function fetchChain() {
    ...
    try {
      // Spot
      const sr = await fetch(`/api/spot?instrument_key=${encodeURIComponent(cfg.instrument_key)}`,
        { headers: token ? { Authorization:`Bearer ${token}` } : {} });
      ...
      if (cd.source === 'upstox' && cd.data?.length) {
        parsed = cd.data.map(item => {
          ...
          const oc=v=>{v=parseFloat(v)||0;return Math.abs(v)<500?v:(parseFloat(cmd.oi)-v)/v*100;};
          return {
            ...
            c_oichg:oc(cmd.prev_oi), p_oichg:oc(pmd.prev_oi),
...
>>>>>>> 7dc49498fac3f7d626dd538895dd11d21fa5fdca
      ...
      setAtm(atmK);
      const ai = parsed.reduce((bi,r,i) => Math.abs(r.strike-atmK)<Math.abs(parsed[bi].strike-atmK)?i:bi, 0);
      setRows(parsed.slice(Math.max(0,ai-10), ai+11));
  ...
  function connectWS(token) {
    fetch('/api/wsauth', { headers:{ Authorization:`Bearer ${token}` } })
      ...
      .then(d=>{
        ...
        ws.onmessage = e => {
          try {
            const msg = typeof e.data==='string' ? JSON.parse(e.data) : JSON.parse(new TextDecoder().decode(e.data));
            const updates={};
            Object.entries(msg.feeds||{}).forEach(([k,f])=>{
              ...
              if(ltp>0) updates[k]={ ltp, oi:parseFloat(m.oi||0), iv:parseFloat(gk.iv||0),
                delta:parseFloat(gk.delta||0), theta:parseFloat(gk.theta||0) };
            });
            if(Object.keys(updates).length) applyUpdates(updates);
          } catch{}
        };
        ws.onclose = () => { setWsLive(false); setWsStatus('Reconnecting...'); setTimeout(()=>connectWS(token),5000); };
...
>>>>>>> 7dc49498fac3f7d626dd538895dd11d21fa5fdca
  ...
  async function submitOrder() {
    ...
    const snapLtp=orderForm.type==='CE'?(cache[row?.c_key]?.ltp||row?.c_ltp||0):(cache[row?.p_key]?.ltp||row?.p_ltp||0);
    ...
    try {
      const r=await fetch('/api/order',{
        ...
        body:JSON.stringify({ instrument_key:key, transaction_type:orderForm.side, quantity:qty,
          price:execPrice, order_type:orderForm.price>0?'LIMIT':'MARKET', product:orderForm.product })
      });
      const d=await r.json();
      const sym=`${indexKey}${orderForm.strike}${orderForm.type}`;
      setOrderMsg(`✅ ${d.status}: ${d.message}`);
      if(d.status!=='ERROR'){
        const pos={sym,side:orderForm.side,qty,entry:execPrice,ltp:execPrice,pnl:0,
          key,time:new Date().toLocaleTimeString('en-IN'),status:d.status};
    ...
    <Layout title="Option Chain" subtitle="Live tick-by-tick data">
      ...
      <div style={{display:'flex',borderBottom:'1px solid var(--border)',marginBottom:0,background:'var(--bg2)',borderRadius:'8px 8px 0 0'}}>
        ...
        {tab==='chain'&&(
          ...
          }}>{showGk?'Hide Greeks':'Show Greeks'}</button>
        )}
      </div>
              ...
              {source==='bse_unavailable' ? (
                <div>
                  ...
                  <div style={{fontSize:12,color:'var(--text3)',maxWidth:320,margin:'0 auto',lineHeight:1.7}}>
                    BSE indices (SENSEX, BANKEX) option chain is not available via NSE API.<br/>
                    <strong style={{color:'var(--gold)'}}>Connect your Upstox token in Settings to get live BSE data.</strong>
                  </div>
                ...
                <div style={{color:'var(--text3)'}}>
                  ...
                  <div style={{fontSize:13}}>No data available. Market may be closed or data fetch failed.</div>
...
>>>>>>> 7dc49498fac3f7d626dd538895dd11d21fa5fdca
                ...
                {rows.map(r=>{
                  ...
                  return [
                    <tr key={r.strike} ref={isAtm?atmRef:null}
                      style={{background:isAtm?'rgba(59,130,246,.11)':'transparent',
                      ...
                      <td className={cF?`flash-${cF}`:''} style={{...S.td,color:'#6ee7b7',fontWeight:700,fontSize:14,textAlign:'right'}}>
                        {fmtNum(cLtp)}
                      </td>
                      ...
                      <td className={pF?`flash-${pF}`:''} style={{...S.td,color:'#fca5a5',fontWeight:700,fontSize:14,textAlign:'left'}}>
                        {fmtNum(pLtp)}
                      </td>
        ...
        <div className="card" style={{borderRadius:'0 0 8px 8px'}}>
          <div className="grid-3" style={{marginBottom:12}}>
            {[
              ...
              ['Strike',<select className="sel" value={orderForm.strike} onChange={e=>setOrderForm(f=>({...f,strike:parseInt(e.target.value)}))}>
                {rows.map(r=><option key={r.strike} value={r.strike}>{r.strike}</option>)}</select>],
      ...
      {tab==='positions'&&(
        <div className="card" style={{borderRadius:'0 0 8px 8px'}}>
          ...
          {positions.length===0
            ?<div style={{padding:24,textAlign:'center',color:'var(--text3)'}}>No open positions. Use B/S buttons to place orders.</div>
            :positions.map((p,i)=>(
              <div key={i} className="card card-sm" style={{marginBottom:8,display:'flex',gap:14,alignItems:'center',flexWrap:'wrap'}}>
                ...
                <div style={{marginLeft:'auto',fontSize:10,color:'var(--text3)'}}>{p.time} · {p.status}</div>
            ...
            :orders.map((o,i)=>(
              <div key={i} style={{display:'flex',gap:12,alignItems:'center',padding:'8px 0',
                ...
                <span style={{fontSize:10,padding:'2px 8px',borderRadius:4,fontWeight:700,
                  background:o.status==='PAPER'?'rgba(251,191,36,.15)':'rgba(74,222,128,.15)',
                  color:o.status==='PAPER'?'var(--gold)':'var(--green)'}}>
                  {o.status}
                </span>
