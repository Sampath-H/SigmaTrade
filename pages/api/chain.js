...
function nse_parse_option_chain(nse_data,instrument_key){
  ...
  for(const K of Object.keys(byStrike).map(Number).sort((a,b)=>a-b)){
    ...
    data.push({strike_price:K,
      call_options:{instrument_key:ce.identifier||`NSE_FO|NIFTY${Math.round(K)}CE`,market_data:{ltp:_s(ce,'lastPrice'),oi:ceOI,prev_oi:cePrevOI,volume:_s(ce,'totalTradedVolume'),close_price:_s(ce,'prevClose'),lot_size:lot},option_greeks:{iv:_s(ce,'impliedVolatility'),delta:_s(ce,'delta'),theta:_s(ce,'theta'),gamma:_s(ce,'gamma'),vega:_s(ce,'vega')}},
      put_options:{instrument_key:pe.identifier||`NSE_FO|NIFTY${Math.round(K)}PE`,market_data:{ltp:_s(pe,'lastPrice'),oi:peOI,prev_oi:pePrevOI,volume:_s(pe,'totalTradedVolume'),close_price:_s(pe,'prevClose'),lot_size:lot},option_greeks:{iv:_s(pe,'impliedVolatility'),delta:_s(pe,'delta'),theta:_s(pe,'theta'),gamma:_s(pe,'gamma'),vega:_s(pe,'vega')}}});
  }
...
export default async function handler(req,res){
  ...
  if(sym){
    try{
      ...
      if(parsed.data.length>0)return res.json({source:'upstox',data:parsed.data,spot:parsed.spot,expiries:parsed.expiries});
