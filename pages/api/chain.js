...
function fmtDate(d) {
  try {
    ...
    if (m[mon]) return `${year}-${m[mon]}-${day.padStart(2, '0')}`;
...
function nse_parse_option_chain(nse_data, instrument_key, expiry_date) {
    ...
    if (!byStrike[K][exp]) {
      ...
      byStrike[K][exp] = {
        ce: {
          ...prev.ce,
          ...Object.fromEntries(
            Object.entries(fce).filter(([, v]) => v !== undefined && v !== null && v !== 0 && v !== '')
          )
        },
        pe: {
          ...prev.pe,
          ...Object.fromEntries(
            Object.entries(fpe).filter(([, v]) => v !== undefined && v !== null && v !== 0 && v !== '')
          )
        }
      };
    }
  ...
  for (const K of Object.keys(byStrike).map(Number).sort((a, b) => a - b)) {
    ...
    data.push({
      strike_price: K,
      call_options: {
        instrument_key: ce.identifier || `NSE_FO|${symbolCode}${Math.round(K)}CE`,
        market_data: {
          ltp: _s(ce, 'lastPrice'),
          oi: ceOI,
          prev_oi: cePrevOI,
      ...
      put_options: {
        instrument_key: pe.identifier || `NSE_FO|${symbolCode}${Math.round(K)}PE`,
        market_data: {
          ltp: _s(pe, 'lastPrice'),
          oi: peOI,
          prev_oi: pePrevOI,
  ...
  return (rows || [])
    .map(row => {
      ...
      return {
        strike_price: strike,
        call_options: {
          instrument_key: ce.identifier || row.c_key || '',
          market_data: {
            ltp: _s(ce, 'lastPrice') || _s(row, 'c_ltp'),
            oi: ceOI,
            prev_oi: (ceChg !== 0 && ceOI > 0) ? ceOI / (1 + ceChg / 100) : ceOI,
            volume: _s(ce, 'totalTradedVolume'),
            close_price: _s(ce, 'prevClose') || _s(row, 'c_prev'),
            lot_size: lot
          },
          option_greeks: {
            iv: _s(ce, 'impliedVolatility') || _s(row, 'c_iv'),
            delta: _s(ce, 'delta') || _s(row, 'c_delta'),
            theta: _s(ce, 'theta') || _s(row, 'c_theta'),
            gamma: _s(ce, 'gamma'),
            vega: _s(ce, 'vega')
          }
        },
        put_options: {
          instrument_key: pe.identifier || row.p_key || '',
          market_data: {
            ltp: _s(pe, 'lastPrice') || _s(row, 'p_ltp'),
            oi: peOI,
            prev_oi: (peChg !== 0 && peOI > 0) ? peOI / (1 + peChg / 100) : peOI,
            volume: _s(pe, 'totalTradedVolume'),
            close_price: _s(pe, 'prevClose') || _s(row, 'p_prev'),
            lot_size: lot
          },
          option_greeks: {
            iv: _s(pe, 'impliedVolatility') || _s(row, 'p_iv'),
            delta: _s(pe, 'delta') || _s(row, 'p_delta'),
            theta: _s(pe, 'theta') || _s(row, 'p_theta'),
            gamma: _s(pe, 'gamma'),
            vega: _s(pe, 'vega')
          }
        }
...
async function nse_fetch_poll_rows(sym) {
  ...
  const rec = d.records || {};
  const fil = d.filtered || {};
  return {
    spot: parseFloat(rec.underlyingValue || fil.underlyingValue || 0),
    rows: (fil.data || rec.data || [])
  };
}
...
async function nse_fetch_option_chain(sym) {
  ...
  await fetch('https://www.nseindia.com/option-chain', {
    headers: { ...h, Cookie: cookies, Accept: 'text/html' }
  });
...
export default async function handler(req, res) {
  ...
  if (instrument_key.startsWith('BSE_INDEX') && (!token || token === 'MOCK_TOKEN')) {
    return res.json({
      source: 'unavailable',
      data: [],
      message: 'BSE option chain requires Upstox token.'
    });
  }
  ...
  if (token && token !== 'MOCK_TOKEN') {
    try {
      ...
      if (r.ok) {
        const d = await r.json();
        const data = d.data || [];
        if (data.length > 0) return res.json({ source: 'upstox', data });
      }
    } catch (e) {
  ...
  if (sym) {
    try {
      ...
      try {
        ...
        console.error('NSE poll fallback error:', e2.message);
      }
    }
  }
