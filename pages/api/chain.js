  ...
  for (const K of Object.keys(byStrike).map(Number).sort((a,b)=>a-b)) {
    ...
    data.push({
      ...
      call_options: {
        ...
        market_data: { ltp: _s(ce,'lastPrice'), oi: ceOI, prev_oi: cePrevOI, volume: _s(ce,'totalTradedVolume'), close_price: _s(ce,'prevClose'), lot_size: lot },
        option_greeks: { iv: _s(ce,'impliedVolatility'), delta: _s(ce,'delta'), theta: _s(ce,'theta'), gamma: _s(ce,'gamma'), vega: _s(ce,'vega') },
      },
      put_options: {
        instrument_key: pe.identifier || `NSE_FO|NIFTY${Math.round(K)}PE`,
        market_data: { ltp: _s(pe,'lastPrice'), oi: peOI, prev_oi: pePrevOI, volume: _s(pe,'totalTradedVolume'), close_price: _s(pe,'prevClose'), lot_size: lot },
        option_greeks: { iv: _s(pe,'impliedVolatility'), delta: _s(pe,'delta'), theta: _s(pe,'theta'), gamma: _s(pe,'gamma'), vega: _s(pe,'vega') },
      },
    });
  }
...
async function nse_fetch_option_chain(sym) {
  ...
  const home = await fetch('https://www.nseindia.com', { headers: { ...headers, Accept: 'text/html,application/xhtml+xml' } });
  const cookieRaw = home.headers.get('set-cookie') || '';
  const cookies = cookieRaw.split(',').map(c => c.split(';')[0].trim()).filter(Boolean).join('; ');
  await fetch('https://www.nseindia.com/option-chain', { headers: { ...headers, Cookie: cookies, Accept: 'text/html' } });
  const r = await fetch(`https://www.nseindia.com/api/option-chain-indices?symbol=${sym}`, { headers: { ...headers, Cookie: cookies } });
  ...
  if (sym) {
    try {
      ...
      if (parsed.data.length > 0) {
        return res.json({ source: 'upstox', data: parsed.data, spot: parsed.spot, expiries: parsed.expiries });
      }
